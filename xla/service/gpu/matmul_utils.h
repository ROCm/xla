...
struct TritonGemmConfig {
  ...
  // MFMA shape selection, nonzero forces AMD's enforced-nonKDim MFMA variant.
  // AMD only, ignored on other backends.
  int matrix_instr_nonkdim = 0;
 ...
 private:
  auto ToTuple() const {
    return std::make_tuple(block_m, block_n, block_k, num_stages, num_warps,
                           ...
                           group_size, matrix_instr_nonkdim);
  }
