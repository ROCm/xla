  ...
  if (shape.IsTuple()) {
    ...
    for (const Shape& element_shape : shape.tuple_shapes()) {
      TF_ASSIGN_OR_RETURN(Literal element,
                          MakeVarLiteral(element_shape, std::move(F)));
...
absl::StatusOr<std::vector<Literal>> MakeSpecialArguments(
  ...
  for (int i = 0; i < params.size(); ++i) {
    TF_ASSIGN_OR_RETURN(
        arguments[i],
        MakeVarLiteral(params[i]->shape(), [](int idx) { return idx + 1; }));
  }
 ...
 protected:
  ...
  void run_internal(std::istream& ifs, std::ostream& ofs,
    ...
    const static std::pair<std::regex, const char*> replaces[] = {
        ...
        {std::regex(R"x(u32\[\] get-dimension-size\()x"),
         "s32[] get-dimension-size("},
        {std::regex(R"x(u32\[\] %get-dimension-size\.)x"),
         "s32[] %get-dimension-size."},
        {std::regex(R"x(u32\[\] get-dimension-size\.)x"),
         "s32[] get-dimension-size."},
    ...
    for (int i = 0; i < num_runs + num_warmups; i++) {
      if (i == num_warmups) {
        ...
        ASSERT_TRUE(
            backend().default_stream_executor()->SynchronizeAllActivity());
      }
      xla::ExecutionProfile profile;
      // profile.set_warmup_run_executed(true);
      TF_ASSERT_OK_AND_ASSIGN(auto result, runner.ExecuteWithDeviceBuffers(
                                               /*executable=*/exec.get(),
                                               /*arguments=*/argument_buffers,
                                               /*profile=*/&profile));
      if (i == 0) {
        TF_ASSERT_OK_AND_ASSIGN(
            auto host_res, runner.TransferLiteralFromDevice(result.Result()));
        // WriteLiteralToTempFile(host_res, name); // write execution results to
        // file
      }
    ...
    TF_ASSERT_OK_AND_ASSIGN(auto test_res, runner.ExecuteWithExecutable(
                                               /*executable=*/exec.get(),
                                               /*arguments=*/arg_ptrs));
    ...
    TF_ASSERT_OK_AND_ASSIGN(
        auto truth, ref_runner.Execute(std::move(ref_module), fake_arguments,
                                       /*run_hlo_passes*/ true));
    ...
    // TF_ASSERT_OK_AND_ASSIGN(
    //      auto ref_exec, ref_runner.CreateExecutable(std::move(ref_module),
    //      true));

    // TF_ASSERT_OK_AND_ASSIGN(
    //      auto truth, ref_runner.ExecuteWithExecutable(ref_exec.get(),
    //      arg_ptrs));
    ...
    // //     absl::Span< xla::Literal * const>(arg_ptrs.data(),
    // arg_ptrs.size()), error_spec));
    ...
    auto fake_arguments =
        xla::MakeFakeArguments(module.get(), true, /*pseudo-random*/
            ...
            .ValueOrDie();
    TF_ASSERT_OK_AND_ASSIGN(
        auto exec, test_runner_.CreateExecutable(std::move(module), true));
...
TEST_F(HloRunnerTest, RunSingle) {
  ...
  for (size_t i = 0; i < matches.size(); i++) {
    ...
    VLOG(0) << i << " of " << matches.size() << ": HLO test for: " << s
            << " ---------------------";
