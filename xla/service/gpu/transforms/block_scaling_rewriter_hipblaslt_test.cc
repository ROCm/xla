...
class BlockScalingRewriterHipblasltTest : public GpuCodegenTest {
 protected:
  ...
  bool IsRocm() const {
    return GetStreamExecutor() != nullptr && GpuCapability().IsRocm();
  }

  void SetUp() override {
    se::StreamExecutor* executor = GetStreamExecutor();
    if (executor == nullptr) {
      GTEST_SKIP() << "No ROCm stream executor available";
    }
    if (!GpuCapability().IsRocm()) {
      GTEST_SKIP() << "Test requires ROCm platform";
    }
    auto rocm_cc = GpuCapability().rocm_compute_capability();
    if (rocm_cc == nullptr || rocm_cc->gfx_version() != "gfx950") {
      GTEST_SKIP() << "Test requires gfx950 architecture";
    }
  }
};
