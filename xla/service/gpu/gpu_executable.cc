...
int64_t UseCachedAllocs() {
  static int64_t value = [] {
    ...
    tsl::ReadInt64FromEnvVar("XLA_COMMAND_BUFFERS_USE_CACHED_ALLOCS", value,
                             &value)
        .IgnoreError();
    return value;
  }();
...
GpuExecutable::GpuExecutable(GpuExecutable::Params params)
  ...
  if (allocs_th > 0) {
    for (const auto& thunk : thunks_->thunks()) {
      ...
      size_t num_nested = 0;
      thunk->ForAllThunks([&num_nested](const Thunk*) { num_nested++; });
...
absl::StatusOr<se::DeviceMemoryBase> GpuExecutable::BufferForAllocation(
  ...
  if (!enable_cached_allocs_) {
    TF_ASSIGN_OR_RETURN(auto buf, memory_allocator->Allocate(
                                      device_ordinal, buffer_size,
                                      /*retry_on_failure=*/true,
                                      /*memory_space=*/allocation.color()));
    VLOG(1) << this << " dev: " << device_ordinal
  ...
  auto [it, inserted] =
      cache.emplace(allocation.index(), AllocationsCacheEntry{});
...
absl::StatusOr<ExecutionOutput> GpuExecutable::ExecuteAsyncOnStreamImpl(
  ...
  if (VLOG_IS_ON(1) && device_ordinal == 0) {
    VLOG(1) << module_name_ << " dev" << device_ordinal
            << " setting for result buf: "
            << result.MutableResult()->on_device_shape().ToString();
  }
  ...
  for (auto& [index, result_buffer] : result.MutableResult()->buffers()) {
    ...
    VLOG(4) << "Looking at: allocation " << output_info.allocation_index
            << " @ index: " << index.ToString() << " output_info.alias_config "
            << output_info.alias_config.has_value();
    ...
    if (enable_cached_allocs_) {
      ...
      if (VLOG_IS_ON(1) && device_ordinal == 0) {
        ...
        VLOG(1) << module_name_ << str << result_buffer.opaque() << " -- "
                << result_buffer.size();
      }
    }
