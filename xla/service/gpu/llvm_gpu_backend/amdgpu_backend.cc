...
bool HsacoCache::find(absl::string_view hash_str, 
  ...
  if (!hit && hsaco_path.has_value()) {
    if (std::ifstream ifs(*hsaco_path, std::ios::binary | std::ios::ate); 
        ifs.is_open()) {
      VLOG(1) << "Hsaco cache hit in file " << *hsaco_path;
      auto hsaco_file_size = ifs.tellg();
      *hsaco = std::vector<uint8_t>(hsaco_file_size);
      ifs.seekg(0, std::ios::beg);
      ifs.read(reinterpret_cast<char*>(hsaco->data()), hsaco_file_size);
      hsaco_cache_.emplace_back(Entry{std::string{hash_str}, *hsaco});
      hit = true;
    }
  }
...
absl::StatusOr<std::vector<uint8_t>> EmitModuleToHsaco(
  ...
  // if file cashe is enabled, this will move hsaco file to the file cache dir
...
absl::StatusOr<std::vector<uint8_t>> CompileToHsaco(
  ...
  }
  {
    ...
    auto lhash = sha256.final();
    absl::string_view hview(reinterpret_cast<const char*>(lhash.data()),
                              lhash.size());
