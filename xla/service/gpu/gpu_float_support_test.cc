...
TEST_F(FloatSupportTest, BF16TranscendentalsOnGfx1250StrictAreNotNormalized) {
  // gfx1250-strict has native bf16 transcendental instructions, so these bf16 ops
  // should be kept as bf16 instead of being upcast to f32.
  auto cc = se::RocmComputeCapability("gfx1250-strict");
  constexpr absl::string_view kHloModule = R"(
HloModule module
...
ENTRY main {
  ...
  for (absl::string_view op : {"exponential", "log", "sqrt", "rsqrt", "tanh"}) {
    ...
    EXPECT_FALSE(
        Normalize(module.get(), se::GpuComputeCapability{cc}, BF16, F32))
        << "bf16 " << op << " should not be normalized on gfx1250-strict";
  }
