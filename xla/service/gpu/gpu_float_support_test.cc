...
TEST_P(StandaloneDotTest, Fp8DotConversion) {
  ...
  TestDotConversion(param.lhs_type, param.rhs_type, param.result_type, cc,
                    param.should_convert_lhs, param.should_convert_rhs,
                    param.low_precision_type);
}
...
TEST_P(TritonFusedDotTest, Fp8DotConversion) {
  const auto& param = GetParam();
  auto cc = se::RocmComputeCapability(param.gpu_version);
  TestTritonFusedDot(param.lhs_type, param.rhs_type, param.result_type, cc,
                     param.should_convert_lhs, param.should_convert_rhs,
                     param.low_precision_type);
}
