...
absl::StatusOr<bool> AutotunerPass::Run(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      Compiler::GetForPlatform(stream_executer_->GetPlatform()));
  std::vector<std::unique_ptr<CodegenBackend>> backends =
      GetAllGpuCodegenBackends(
          stream_executer_,
          /*debug_options=*/&module->config().debug_options(), compiler.get());
  std::unique_ptr<GpuProfiler> profiler =
      GpuProfiler::Create(stream_executer_, ProfileOptions());
  TF_ASSIGN_OR_RETURN(
      std::unique_ptr<Autotuner> autotuner,
      Autotuner::Create(std::move(backends), std::move(profiler),
                        AutotuneConfig(), thread_pool_));
  ...
  TF_RETURN_IF_ERROR(autotuner->Autotune(module, should_autotune));
  return true;
}
