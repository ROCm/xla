...
bool ContractingShouldBeMinor(const DotOperandDims& concat_before) {
  ...
  const int64_t minor_dim = shape.has_layout()
                                ? shape.layout().minor_to_major(0)
                                : rank - 1;
...
absl::StatusOr<ConcatTargetInfo> TransformConcatDims(
  ...
  const std::array<DotOperandDims::Category, 3> category_order =
      contracting_is_minor
          ? std::array<DotOperandDims::Category, 3>{DotOperandDims::kBatch,
...
absl::StatusOr<std::vector<ConcatTargetInfo>> ComputeTargetConcatDims(
  ...
  ASSIGN_OR_RETURN(DotOperandDims first_concat_dims,
                   DotOperandDims::FromDotOperand(
                       dots_to_merge[0].dot,
