...
TEST_F(TilePropagationTest, CanPropagateToInputOfConvOpNHWC) {
  HloInstruction* root = ParseAndGetRoot(R"(
    HloModule m
    ENTRY e {
  ...
  EXPECT_EQ(tiling_space->num_dimensions(), 7);
...
TEST_F(TilePropagationTest, CanPropagateToInputOfConvOpNCHW) {
  HloInstruction* root = ParseAndGetRoot(R"(
    HloModule m
    ENTRY e {
  ...
  EXPECT_THAT(tiled_operands, MatchToString(R"(
    0) (tid_0, tid_1, tid_2, tid_3, tid_4, tid_5, tid_6)
         -> offsets [tid_0 * ts_0, tid_6 * ts_6,
                     tid_2 * ts_2 + tid_4 * ts_4, tid_3 * ts_3 + tid_5 * ts_5]
            sizes [ts_0, ts_6, ts_2, ts_3]
            strides [1, 1, 1, 1]
            upper bounds [1, 4, 5, 5]
    1) (tid_0, tid_1, tid_2, tid_3, tid_4, tid_5, tid_6)
         -> offsets [tid_1 * ts_1, tid_6 * ts_6, tid_4 * ts_4, tid_5 * ts_5]
            sizes [ts_1, ts_6, ts_4, ts_5]
            strides [1, 1, 1, 1]
            upper bounds [8, 4, 3, 3]
  )"));
}
