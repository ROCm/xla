...
absl::StatusOr<TensorValue> EmitAllGather(
  ...
  // AllGather-start returns a tuple (input, output); use the output shape
  // (element 1) to determine the output element type.
  const Shape& all_gather_output_shape =
      all_gather.shape().IsTuple()
          ? ShapeUtil::GetTupleElementShape(all_gather.shape(), 1)
          : all_gather.shape();

  ASSIGN_OR_RETURN(auto output_element_type,
                   xtile::PrimitiveTypeToMlirType(
                       b, all_gather_output_shape.element_type()));

  // Use the tiled output shape from TiledHloComputation (set by experimental
  // tiling via TiledHloInstruction::tile_sizes()).
...
absl::StatusOr<TensorValue> EmitTiledHloInstruction(
  ...
  if (hlo->opcode() == HloOpcode::kAllGatherStart) {
    ...
    // Handle GetTupleElement wrapping (AllGather-start returns a tuple).
