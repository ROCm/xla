...
absl::Status TritonToLLVM(mlir::ModuleOp module, absl::string_view arch_name,
  ...
  xla::gpu::CreateTritonPipeline(
      &pm, stream_executor::GpuComputeCapability(rocm_cc), num_warps, num_ctas,
      num_stages);
...
absl::StatusOr<std::string> LLVMToHSACO(mlir::ModuleOp module,
  ...
  TF_ASSIGN_OR_RETURN(
      auto hsaco_file_result,
      xla::gpu::amdgpu::CompileToHsacoAndReturnFilePath(
          llvm_module.get(), stream_executor::GpuComputeCapability(rocm_cc),
          debug_opts, /*keep_tempfiles=*/false));
...
absl::StatusOr<CompilationResult> Compile(absl::string_view module,
  ...
  return CompilationResult{
      HsacoPath{hsaco_path}, shared_mem_bytes,
  };
}
