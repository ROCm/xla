...
class LocalDeviceState {
 public:
  ...
  struct StreamOptions {
    ...
    // only one thread will create the stream.
  ...
  // the hipStreamCreate overhead again. Specifically:
 ...
 private:
  ...
  // This helper is used by both eager (constructor) and lazy (first-access)
  // stream creation paths. It is safe to call from const methods.
  ...
  // overhead on the hot path after initialization.
  //
  // Declared mutable so that compute_stream() and host_to_device_stream(),
  // which are const methods, can lazily initialize them.
  ...
  // created. Read without locks; written once under lazy_init_mu_.
  // memory_order_acquire / memory_order_release ordering ensures the
  // stream object is fully visible before the pointer becomes non-null.
  ...
  // (nullopt means use executor default).
  ...
  // that const methods (compute_stream, host_to_device_stream) can use it for
  // lazy initialization of vector stream slots.
