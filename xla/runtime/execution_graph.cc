...
ExecutionGraph::ExecutionGraph(NodesEdges nodes_in_edges,
  ...
  // Check if constructed execution DAG is sequential: every node depends on
  // the completion of the previous node.
  ...
  for (NodeId i = 0; i < operations.size(); ++i) {
    ...
    for (NodeId j = 0; j < i; ++j) {
      if (buffer_rwsets[j].HasConflicts(buffer_rwsets[i])) {
        ...
        // accessed by both nodes to find out what kind of edge we need to
        // add.
...
ExecutionGraph::CreateNodeDefs(std::vector<NodeDefBuilder> builders) {
  ...
  // Reserve memory to avoid re-allocation and dangling spans into freed
  // memory.
...
int64_t ExecutionGraph::EraseEdge(NodeDefBuilder& from, NodeDefBuilder& to,
  ...
  // Short-circuit if out-edges or in-edges don't intersect with `to` or
  // `from` node ids (remember that edges are sorted).
  ...
  // At this point we must have exactly one edge between `from` and `to`
  // nodes.
...
int64_t ExecutionGraph::RunTransitiveReductionAndUpdatePriorities(
  ...
  for (int64_t i = builders.size() - 1; i >= 0; --i) {
    ...
    for (NodeEdge out_edge : std::vector<NodeEdge>(source_node.out_edges)) {
      ...
      // Initialize state with nodes reachable via `out_edge`. We mark
      // immediate out nodes as visited to correctly compute node priority
      // below.
