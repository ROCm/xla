...
class CommandBuffer {
 public:
  ...
  virtual void SkipUpdates(ExecutionScopeId execution_scope_id, int64_t num = 1) = 0;

  virtual absl::StatusOr<size_t> GetNumChildNodes() const = 0;

  virtual absl::StatusOr<std::unique_ptr<CommandBuffer>> Clone() = 0;
  //--------------------------------------------------------------------------//
