...
XgmiTopologyInfo GetRocmXgmiTopology(absl::string_view pci_bus_id) {
  ...
  if (!bdf.has_value()) {
    LOG(WARNING) << "Failed to parse PCI bus ID for xGMI query: "
                 << pci_bus_id;
    return info;
  }
  ...
  rsmi_status_t status =
      wrap::rsmi_dev_xgmi_hive_id_get(*dev_idx, &hive_id);
  ...
  // This mirrors the CUDA approach of iterating NVLink lanes and checking
  // P2P capability, but uses pairwise GPU topology queries instead.
  uint32_t num_devices = 0;
  status = wrap::rsmi_num_monitor_devices(&num_devices);
  ...
  for (uint32_t i = 0; i < num_devices; ++i) {
    ...
    status = wrap::rsmi_topo_get_link_type(*dev_idx, i, &hops, &link_type);
  ...
  VLOG(1) << "xGMI topology for " << pci_bus_id << ": "
