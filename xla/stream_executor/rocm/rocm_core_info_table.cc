...
const CoreInfoRows* FindRocmCoreInfo(const RocmComputeCapability& cc) {
  ...
  // units_per_core=4 on matrix rows. Vector rows use units_per_core = the count
  // of FP32-FMA-equivalent lanes such that units * ops * 2 reproduces the
...
int RocmFpusPerCoreFallback(const RocmComputeCapability& /*cc*/) {
  // Used when the gfx target isn't in the table yet. Historically all ROCm
