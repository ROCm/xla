...
absl::StatusOr<BlasLt::MatmulPlanPtr> BlasLt::GetGroupedMatmulPlan(
  ...
  std::vector<int64_t> v_m(cfg.group_count, cfg.m), v_n(cfg.group_count, cfg.n),
      v_k(cfg.group_count, cfg.k),
  ...
  auto grouped_gemm = std::make_unique<hipblaslt_ext::GroupedGemm>(
      handle_.get(), problem.getOpA(), problem.getOpB(), problem.getTypeA(),
      problem.getTypeB(), problem.getTypeC(), problem.getTypeD(),
  ...
  return std::make_unique<GroupedMatmulPlan>(*this, std::move(cfg),
                                             std::move(grouped_gemm));
}
