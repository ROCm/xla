...
// Fills the scalar and matrix unit fields in `desc` with Stream Processor and
// Matrix Core descriptions if available for the given ROCm compute capability.
void FillExecutionUnitDesc(const RocmComputeCapability& cc,
                           float base_clock_rate_ghz,
                           DeviceDescription& desc);

// Gets the number of FPUs (Stream Processors) per CU. Assumes FP32 units.
