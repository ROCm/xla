...
// hip_bfloat16 is a distinct type different from Eigen::bfloat16.
// CUDA doesn't provide std::numeric_limits<hip_bfloat16>::infinity(), which
// makes it default to returning 0. This is suboptimal and results in a lot of
// fun when debugging.
//
// Neither the constructor of hip_bfloat16 nor the CUDART_INF_BF16 constants
...
// - hip_bfloat16 and Eigen::bfloat16 are bitwise identical
// So we can get a constant this way.
template <>
__host__ __device__ constexpr hip_bfloat16 kInfinity<hip_bfloat16> =
...
__device__ inline bool IsNan(hip_bfloat16 v) { return __isnan(v); }
__device__ inline bool IsInf(float v) { return isinf(v); }
__device__ inline bool IsInf(double v) { return isinf(v); }
__device__ inline bool IsInf(hip_bfloat16 v) { return __isinf(v); }
...
__device__ uint32_t WarpReduceSum(uint32_t value) {
  static constexpr uint64_t kFullMask = ~0UL;
...
__global__ void ReduceFloatCheckResults(
  ...
  if (BlockIdx() == 0 && ThreadIdx() == 0) {
    const uint32_t write_idx = atomicAdd_system(&log_header->write_idx, 1);
...
se::KernelLoaderSpec GetFloatCheckBf16KernelSpec(int arity) {
  return se::KernelLoaderSpec::CreateInProcessSymbolSpec(
      absl::bit_cast<void*>(&FloatCheck<hip_bfloat16>),
