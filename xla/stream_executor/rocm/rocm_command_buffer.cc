...
absl::StatusOr<GraphNodeHandle> RocmCommandBuffer::CopyChildNodeToMainGraph(
  ...
  }
  {
    hipMemsetParams params;
    TF_RETURN_IF_ERROR(
        ToStatus(wrap::hipGraphMemsetNodeGetParams(hchild, &params),
                 "CopyChildNodeToMainGraph failed getting memset node params"));
    TF_RETURN_IF_ERROR(
        ToStatus(wrap::hipGraphAddMemsetNode(&hmain, graph_, deps.data(),
                                             deps.size(), &params),
...
absl::Status RocmCommandBuffer::UpdateChildNodeInMainGraph(
...

  {
    ...
    if (s == hipSuccess) {
      return ToStatus(
          wrap::hipGraphExecKernelNodeSetParams(exec_, hmain, &params));
    }
  }
  {
    hipMemsetParams params;
...
static struct FinalPrinter {
 ...
 private:
  using Map =
      std::unordered_map<GpuGraphHandle, std::tuple<std::string, uint64_t>>;
...
absl::StatusOr<size_t> RocmCommandBuffer::GraphGetNodes(
  ...
  auto* hipnodes =
      reinterpret_cast<hipGraphNode_t*>(pnodes ? pnodes->data() : nullptr);
  TF_RETURN_IF_ERROR(
      ToStatus(wrap::hipGraphGetNodes(graph_, hipnodes, &numNodes),
...
std::unique_ptr<stream_executor::gpu::ScopedUpdateMode>
RocmCommandBuffer::ActivateUpdateMode(GpuCommandBuffer* nested_cmd_buffer) {
