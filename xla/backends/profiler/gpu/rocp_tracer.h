...
#ifndef XLA_BACKENDS_PROFILER_GPU_ROCP_TRACER_H_
#define XLA_BACKENDS_PROFILER_GPU_ROCP_TRACER_H_
...
struct RocpTracerOptions {
  // maximum number of annotation strings that AnnotationMap in RocpTracer can
  // store. e.g. 1M
...
// only one RocpTracer per process.
class RocpTracer {
 public:
  // Returns a reference to the singleton instance of RocpTracer.
  ...
  void Enable(const RocpTracerOptions& options, RocmTraceCollector* collector_);
  void Disable();
 ...
 private:
  ...
  std::optional<RocpTracerOptions> options_;
 ...
 public:
  ...
  RocpTracer(const RocpTracer&) = delete;
  RocpTracer& operator=(const RocpTracer&) = delete;
};

}  // namespace profiler
}  // namespace xla

#endif  // XLA_BACKENDS_PROFILER_GPU_ROCP_TRACER_H_
