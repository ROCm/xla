...
struct AmdGpuTargetConstants {
  ...
  uint32_t total_sgprs;        // AMDGPU::getTotalNumSGPRs
  uint32_t sgpr_granule;       // AMDGPU::getSGPRAllocGranule
...
//
// Exposed rather than kept private because ToXStat() needs the same answer for
// the `regs:` token it puts next to occ_pct, and because the max() branch is
// load-bearing rather than defensive -- see the gfx908 trap documented in the
// .cc.
...
// CONSUMERS. Only occupancy_pct, total_vgprs and min_grid_size are read by
// rocm_collector.cc today; the rest are computed for, and currently asserted
// only by, rocm_occupancy_test.cc. `limiter` in particular costs ~30 lines of
// the subtlest reasoning in the .cc and has no production reader yet -- it is
// here because it is the number an engineer can act on, and surfacing it as an
// XStat is the next step. If that step is abandoned, delete the attribution
...
struct OccupancyStats {
  ...
  // Blocks needed to fill the whole DEVICE at this occupancy -- CUDA's
  // min_grid_size units. 0 if cu_count was unknown.
...
// size, no register data at all, or input the version-skew guard rejects. The
// caller must then emit no occupancy stats, NOT zero ones.
