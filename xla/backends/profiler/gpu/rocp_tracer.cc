...
// ----------------------------------------------------------------------------
// Stub implementations for RocpTracer static functions expected by
// rocprofiler-sdk.
// ----------------------------------------------------------------------------
RocpTracer& RocpTracer::i() {
  static RocpTracer obj;
  return obj;
}
...
/*static*/ uint64_t RocpTracer::GetTimestamp() {
  uint64_t ts;
...
void RocpTracer::Enable(const RocpTracerOptions& options,
...
void RocpTracer::TracingCallback(rocprofiler_context_id_t context,
...
void RocpTracer::CodeObjectCallback(
...
static void code_object_callback(rocprofiler_callback_tracing_record_t record,
                                 ...
                                 void* callback_data) {
  RocpTracer::i().CodeObjectCallback(record, callback_data);
}
...
static void tool_tracing_callback(rocprofiler_context_id_t context,
                                  ...
                                  uint64_t drop_count) {
  RocpTracer::i().TracingCallback(context, buffer_id, headers, num_headers,
                                  drop_count);
}

int RocpTracer::toolInit(rocprofiler_client_finalize_t fini_func,
                         void* tool_data) {
  // Gather API names
    ...
    rocprofiler_configure_callback_tracing_service(
        ...
        [](rocprofiler_callback_tracing_record_t record,
          ...
          if (record.phase == ROCPROFILER_CALLBACK_PHASE_ENTER) {
            ...
            if (!annotation.empty()) {
              RocpTracer::i().annotation_map()->Add(
...
void RocpTracer::toolFinalize(void* tool_data) {
  auto& obj = RocpTracer::i();
...
static int toolInitStatic(rocprofiler_client_finalize_t finalize_func,
                          void* tool_data) {
  return RocpTracer::i().toolInit(finalize_func, tool_data);
}
...
extern "C" rocprofiler_tool_configure_result_t* rocprofiler_configure(
  ...
  auto& obj = RocpTracer::i();  // Ensure constructed, critical for tracing.
  ...
  static rocprofiler_tool_configure_result_t cfg{
      ...
      &RocpTracer::toolFinalize, nullptr};
