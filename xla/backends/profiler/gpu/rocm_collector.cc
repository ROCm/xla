...
std::vector<RocmTracerEvent> RocmTraceCollectorImpl::ApiActivityInfoExchange() {
  ...
  */
  ...
  // Pre-reserve to avoid reallocations; there can be many activity events
  // per correlation_id (e.g., hipGraphLaunch).
  size_t total_activity_events = 0;
  for (const auto& kv : activity_ops_events_map_) {
    total_activity_events += kv.second.size();
  }
  LOG(WARNING) << "!!!total_activity_events = " << total_activity_events;
  ...
  for (auto& [key, api_event] : api_events_map_) {
    ...
    // Use the first activity as a representative for device/stream info.
    ...
    switch (api_event.type) {
      case RocmTracerEventType::Kernel:
        // For the host/API event, keep the behavior of attaching some
        // kernel_info derived from the activity.
        api_event.kernel_info = item.kernel_info;
        aggregated_events.push_back(api_event);
        break;
  ...
  //    This is where we fix the "only first kernel" problem.
  // ---------------------------------------------------------------------------
  for (auto& activity_iter : activity_ops_events_map_) {
    ...
    // Find the corresponding API event (primary or auxiliary).
    ...
    if (api_event == auxiliary_api_events_map_.end()) {
      // No matching API information: drop all activity events for this corr_id.
    ...
    for (auto& activity_event : activity_vec) {
      switch (activity_event.type) {
        case RocmTracerEventType::Kernel:
#if defined(XLA_GPU_ROCM_TRACER_BACKEND) && (XLA_GPU_ROCM_TRACER_BACKEND == 1)
          // For roctracer backend, the actual kernel name may only be known
...
#endif
          // IMPORTANT:
          //   - For rocprofiler-sdk, each activity_event should already carry
          //     its own kernel_info (grid/workgroup/private/group mem).
          //   - For roctracer, if kernel_info is empty on the activity side,
          //     we fall back to api_event's kernel_info.
        ...
        case RocmTracerEventType::MemcpyD2D:
        case RocmTracerEventType::MemcpyOther:
          // We keep memcpy_info as provided by the activity record.
          aggregated_events.push_back(activity_event);
          break;
