...
void PerDeviceCollector::GetDeviceCapabilities(
  ...
  // Agent clocks are in MHz; profiler expects KHz.
  ...
  if (core_count) {
    device_plane->AddStatValue(*device_plane->GetOrCreateStatMetadata(
                               ...
                               static_cast<int64_t>(core_count));
  }

  // Sum VRAM-only memory banks and use the first bank's clock/width.
  uint64_t total_memory = 0;
  uint32_t mem_clock_mhz = 0;
  uint32_t mem_bus_width_bits = 0;
  for (uint32_t i = 0; i < agent.mem_banks_count; ++i) {
    if (agent.mem_banks[i].heap_type == HSA_HEAPTYPE_FRAME_BUFFER_PUBLIC ||
      ...
      if (mem_clock_mhz == 0) {
        mem_clock_mhz = agent.mem_banks[i].mem_clk_max;
        mem_bus_width_bits = agent.mem_banks[i].width;
      }
    }
  }

  if (mem_clock_mhz && mem_bus_width_bits) {
    auto memory_bandwidth =
        uint64_t{2} * mem_clock_mhz * 1000000ULL * mem_bus_width_bits / 8;
    device_plane->AddStatValue(
        *device_plane->GetOrCreateStatMetadata(
