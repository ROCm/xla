...
class FissionBackend : public GpuCodegenBackend {
 public:
  // Builds a fresh rewriter pipeline. CodegenOrchestrator may invoke
  // FissionBackend's methods concurrently, and HloPassPipeline is not
  // reentrant, so a factory is used instead of a shared pipeline instance.
