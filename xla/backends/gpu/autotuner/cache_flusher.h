...
//
// This evicts the input, output and scratch buffers alike, and costs one
// fixed allocation for the whole autotuning pass rather than one per
// instruction.
class CacheFlusher {
 public:
  // Allocates the scratch buffer and loads the kernel. `flush_bytes` should
  // exceed the last level cache. Returns an error if either the allocation
  // or the kernel load fails, in which case the caller should proceed
  // without flushing rather than fail compilation.
