...
struct MatmulPlanCache {
  ...
  template <class Func>
  absl::StatusOr<se::gpu::BlasLt::MatmulPlan*> GetOrCreate(
      const std::string& key, Func&& create) {
...
auto CublasLtMatmulThunk::GetCachedMatmulPlan(const ExecuteParams& params)
