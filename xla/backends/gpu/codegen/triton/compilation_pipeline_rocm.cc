...
static void MakeTTGIR(mlir::OpPassManager* pm,
  ...
  bool use_async_copy = is_async_copy_enabled(rocm_cc, rocm_use_async_copy);
...
void CreateTritonRocmPipeline(
    ...
    int num_ctas, int num_stages, std::optional<bool> rocm_use_async_copy) {
  MakeTTIR(pm, rocm_cc);
  MakeTTGIR(pm, rocm_cc, num_warps, num_ctas, num_stages, rocm_use_async_copy);
