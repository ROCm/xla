...
// Backend-specific options for the Triton compilation pipeline.
struct TritonPipelineOptions {
  struct RocmOptions {
    // If unset, use Triton's architecture-specific default.
    std::optional<bool> use_async_copy = std::nullopt;
  };
...
void CreateTritonPipeline(mlir::OpPassManager* pm,
                          ...
                          const TritonPipelineOptions& options = {});
