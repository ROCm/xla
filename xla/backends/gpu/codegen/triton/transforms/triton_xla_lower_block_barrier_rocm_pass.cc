...
// this ROCm version uses tensor operations where each element represents a
...
LogicalResult LowerBlockBarrierOpROCm(BlockBarrierOp block_barrier,
  ...
  VLOG(2) << "LowerBlockBarrierOpROCm: Starting ROCm-specific block barrier "
             "lowering";
  ...
  VLOG(3) << "LowerBlockBarrierOpROCm: world_size=" << world_size;

  // Triton magic constant for global address space
  constexpr int32_t kGlobalAddressSpace = 1;

  VLOG(3) << "LowerBlockBarrierOpROCm: Creating types";
  ...
  VLOG(3) << "LowerBlockBarrierOpROCm: Getting block ID";
  ...
  VLOG(3) << "LowerBlockBarrierOpROCm: Block ID created successfully";
  ...
  VLOG(3) << "LowerBlockBarrierOpROCm: Creating AtomicWriteOp for signaling";

  // Create mask for valid ranks (all true for world_size elements)
  auto mask_tensor_type =
      mlir::RankedTensorType::get({world_size}, builder.getI1Type());
  auto true_attr = builder.getBoolAttr(true);
  auto mask_tensor = mlir::arith::ConstantOp::create(
      builder, mlir::DenseElementsAttr::get(mask_tensor_type, true_attr));
  ...
  VLOG(3) << "LowerBlockBarrierOpROCm: Creating atomic spin-wait operations";
