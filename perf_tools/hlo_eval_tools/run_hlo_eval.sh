...
#
# For each leaf, every module is validated to use the same device/partition count
# N (`num_partitions=N`, default 1). Modules with N>1 were lowered by JAX with
# Shardy, so they run with `--num_partitions=N --use_shardy_partitioner` and
...
# Expected native/legacy CSV header for a set of HLO files.
expected_csv_header() {
  local header="Datetime" file
...
# First non-comment, non-empty CSV line.
csv_header() {
  awk '!/^[[:space:]]*#/ && NF {print; exit}' "$1"
}

# Number of data rows after the first non-comment header line.
csv_timing_rows() {
  awk '!/^[[:space:]]*#/ && NF {rows++} END {print (rows > 0 ? rows - 1 : 0)}' "$1"
}

# Validate that every non-comment CSV row has the expected number of fields and
# that every timing field is a complete numeric millisecond value.
csv_is_complete() {
  local file=$1 expected_columns=$2 minimum_rows=${3:-0}
  [ -s "$file" ] && [ "$(tail -c 1 -- "$file" | wc -l)" -eq 1 ] || return 1
  awk -F, -v expected="$expected_columns" -v minimum="$minimum_rows" '
    function trim(value) {
    ...
    }
    {
      ...
      for (column = 2; column <= NF; column++) {
        value = trim($column)
        if (value !~ /^([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?ms$/) {
          exit 1
        }
      }
      rows++
    ...
    END {
      if (!header || rows < minimum) exit 1
    }
  ' "$file"
}

# Expected partition count encoded by an inference/<N>gpu parent path.
inference_path_parts() {
  local path=$1 absolute
  [ -f "$path" ] && path=$(dirname -- "$path")
  absolute=$(cd -- "$path" 2>/dev/null && pwd -P) || absolute=${path%/}
  if [[ "$absolute" =~ /inference/([0-9]+)gpu$ ]]; then
    printf '%s' "${BASH_REMATCH[1]}"
  fi
}

validate_path_parts() {
  local path=$1 actual=$2 csv=$3 expected
  expected=$(inference_path_parts "$path")
  if [ -n "$expected" ] && [ "$actual" != "$expected" ]; then
    FAILED+=("$csv")
    echo "  FAIL: inference path expects $expected partition(s), HLO header has $actual: $path"
    return 1
  fi
  return 0
}
...
invoke() {
  ...
  local expected_header; expected_header=$(expected_csv_header "$@")
  local expected_columns=$(( $# + 1 ))
  local previous_rows=0
  mkdir -p "$tmp_dir"
  rm -f -- "$tmp_file"
  ...
  if [ -s "$final_file" ]; then
    if [ "$(csv_header "$final_file")" != "$expected_header" ] ||
       ! csv_is_complete "$final_file" "$expected_columns" 1; then
      FAILED+=("$csv")
      echo "  FAIL: existing CSV is incomplete or does not match selected HLOs: $final_file"
      rmdir -- "$tmp_dir" 2>/dev/null || true
      return
    fi
    previous_rows=$(csv_timing_rows "$final_file")
  ...
  if [ "$rc" -eq 0 ] && [ "$parse_rc" -eq 0 ] && [ -s "$tmp_file" ]; then
    local actual_header actual_rows
    actual_header=$(csv_header "$tmp_file")
    actual_rows=$(csv_timing_rows "$tmp_file")
    if [ "$actual_header" != "$expected_header" ] ||
       [ "$actual_rows" -ne $((previous_rows + 1)) ] ||
       ! csv_is_complete "$tmp_file" "$expected_columns" $((previous_rows + 1)); then
      publish_rc=1
      echo "  FAIL: incomplete or unexpected CSV output: $tmp_file"
    else
...
run_leaf() {
  ...
  validate_path_parts "$leaf" "$n" "$csv" || return
  if [ -z "$SINGLE_CSV" ] && [ "$RESUME" != 0 ] && [ -s "${csv}.csv" ]; then
    if [ "$(csv_header "${csv}.csv")" != "$(expected_csv_header "${files[@]}")" ] ||
       ! csv_is_complete "${csv}.csv" $(( ${#files[@]} + 1 )) 1; then
      FAILED+=("$csv")
      echo "  FAIL: resume CSV is incomplete or does not match selected HLOs: ${csv}.csv"
      return
    fi
...
if [ -f "$HLO" ]; then
  ...
  echo "file: $HLO"
  validate_path_parts "$HLO" "$n" "$csv" && invoke "$n" "$csv" "$HLO"
else
