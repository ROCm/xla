...
### Scheduled / event-driven workflows
    ...
    | `therock_xla_ci_nightly.yml` | Nightly at 5 AM UTC                                               | Runs the latest XLA (openxla/xla) and latest JAX (jax-ml/jax) unit tests suite and sanitizers TSAN, ASAN (using `therock_xla_ci_sanitizers.yml` script) on the latest theRock nightly/prereleases. Currently pinned to version 10.0 prerelease |
