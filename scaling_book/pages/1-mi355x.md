...
![AMD MI350 Series package with two IODs, eight XCDs, eight HBM3E stacks, PCIe, and Infinity Fabric links](img/pg1/mi350-arch-diagram.png)

*AMD's MI350 Series package diagram, which covers the MI355X organization used here: two IODs connect eight XCDs, eight HBM3E stacks, host PCIe, and external Infinity Fabric. The diagram's 1,075 GB/s Infinity Fabric figure is aggregate bidirectional bandwidth.*

![An XCD with CUs and 32 KiB L1 caches, a shared 4 MB L2, four ACE compute accelerators, hardware queues, and a hardware scheduler](img/pg1/xcd.png)

*AMD's XCD diagram, which covers one of the eight compute dies in the MI355X organization used here: each CU has a 32 KiB L1, the die shares a 4 MB L2, and four ACE compute accelerators sit above hardware queues beside the hardware scheduler. The diagram labels CUs through CU39; the counts used here are 36 physical CUs per XCD, 32 of them active.*
...
![The ROCm device stack from a JAX logical device through PJRT, StreamExecutor, HIP, HSA, and the amdgpu kernel driver to an MI355X](img/pg1/rocm-device-stack.png)

*How one physical MI355X becomes a logical JAX device. The full-chip SPX mode normally presents one OAM as one device such as rocm:0. Container mappings, HIP_VISIBLE_DEVICES, and hardware partition mode can change that enumeration. JAX sees the logical device, not its IOD, XCD, CU, or wavefront hierarchy.*
...
![A Transformer projection lowering through dot_general and a GEMM kernel into workgroup macrotiles and wave-level MFMA instructions](img/pg1/gemm-to-mfma.png)

*The abstraction narrows from a model projection to a wave-level matrix instruction. Tile sizes in the diagram are symbolic because hipBLASLt, Triton, AITER, and XLA can select different macrotiles and MFMA forms for the same JAX contraction.*
...
![A matrix multiplication shown as a scalar inner product and as an outer-product update over one K slice](img/pg1/gemm-inner-outer.png)

*One output element is a row-column inner product; one K slice is an outer-product update across the output. An MFMA performs the blocked matrix form, updating many partial dot products together.*
...
![Eight AMD MI350X accelerators on a UBB 2.0 baseboard](img/pg1/8socket-mi350.png)

*AMD's eight-OAM MI350 platform. The accelerators pictured are MI350X rather than MI355X, but both use the same CDNA 4 package organization and UBB 2.0 baseboard layout discussed here.*
...
![Generic two-tier network connecting groups of GPU nodes through leaf and spine switches](img/pg1/amd-2tier-design.png)

*A generic two-tier scale-out design. The diagram shows the leaf-and-spine hierarchy rather than a fixed MI355X configuration; NIC models, switch counts, rail placement, and subscription ratios depend on the deployed system.*
...
![Two eight-GPU MI355X nodes connected through a rail-optimized leaf-and-spine network](img/pg1/multinode-topology.png)

*A non-normative two-node rail example. Each node contains an eight-GPU xGMI full mesh, while one external NIC per GPU connects equal GPU indices to the same leaf. The highlighted paths contrast a same-rail transfer with cross-rail traffic that reaches the spine. Pollara 400 is the reference NIC used in this schematic.*
